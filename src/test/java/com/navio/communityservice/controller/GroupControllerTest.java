...
@org.springframework.context.annotation.Import(com.navio.communityservice.security.CommunitySecurityConfig.class)
...
class GroupControllerTest {
    ...
    @Test void guestsCanReadAndLiteralsRouteCorrectly() throws Exception {
        ...
        mvc.perform(get("/v1/groups/mine").with(jwt().jwt(j -> j.subject(user.toString())))).andExpect(status().isOk());
    ...
    @Test void validationRejectsBlankLongAndMalformedBodies() throws Exception {
        for (String name : List.of(" ", "x".repeat(121))) {
            mvc.perform(post("/v1/groups").with(jwt().jwt(j -> j.subject(user.toString()))).contentType(MediaType.APPLICATION_JSON)
                    .content("{\"name\":\"" + name + "\",\"description\":\"tips\"}"))
                    .andExpect(status().isBadRequest()).andExpect(jsonPath("$.validationErrors.name").exists());
        }
        mvc.perform(post("/v1/groups").with(jwt().jwt(j -> j.subject(user.toString()))).contentType(MediaType.APPLICATION_JSON).content("{"))
                .andExpect(status().isBadRequest());
        mvc.perform(get("/v1/groups").header("X-User-Id", "not-a-uuid")).andExpect(status().isOk());
        ...
        mvc.perform(patch("/v1/groups/ev/members/me").with(jwt().jwt(j -> j.subject(user.toString())))
                .contentType(MediaType.APPLICATION_JSON).content("{}"))
                .andExpect(status().isBadRequest());
    }
    @Test void profileRejectsImmutableFieldsAndFlairsValidateTone() throws Exception {
        mvc.perform(patch("/v1/groups/ev/profile").with(jwt().jwt(j -> j.subject(user.toString()))).contentType(MediaType.APPLICATION_JSON)
                .content("{\"slug\":\"new-name\"}"))
                .andExpect(status().isBadRequest());
        mvc.perform(put("/v1/groups/ev/flairs").with(jwt().jwt(j -> j.subject(user.toString()))).contentType(MediaType.APPLICATION_JSON)
    ...
    @Test void domainErrorsKeepConsistentShape() throws Exception {
        ...
        mvc.perform(post("/v1/groups/ev/members/me").with(jwt().jwt(j -> j.subject(user.toString()))))
